...
app.get('/oauth2callback', async (req, res) => {
    ...
    try {
        ...
        await pool.query('INSERT INTO users (id, tokens) VALUES ($1, $2) ON CONFLICT (id) DO UPDATE SET tokens = $2', [userId, tokens]);
...
app.post('/generate-link', async (req, res) => {
    ...
    if (!title || !duration || !availability || !Array.isArray(availability) || !startAddress) {
        return res.status(400).send('Ongeldige invoer. Alle velden zijn verplicht.');
    }
    ...
    try {
        ...
        await pool.query(
            ...
            [linkId, userId, title, parseInt(duration, 10), parseInt(buffer, 10) || 0, JSON.stringify(availability), startAddress]
        );
        res.json({ linkId });
    } catch (error) {
...
app.get('/get-availability', async (req, res) => {
    ...
    console.log(`Request received with linkId: ${linkId} and destinationAddress: ${destinationAddress}`);
    ...
    try {
        ...
        console.log(`Found link for user ${userId} with start address: ${startAddress}`);
        ...
        if (!tokens || !tokens.refresh_token) {
            ...
            return res.status(500).send('Fout in de gebruikersauthenticatie. Probeer opnieuw in te loggen.');
        }
        ...
        console.log(`Appointment details: duration=${appointmentDurationMinutes}min, buffer=${bufferMinutes}min`);
        ...
        while (currentDay < new Date(timeMax)) {
            ...
            if (rule) {
                ...
                while (slotStart < dayEnd) {
                    const previousAppointment = [...busySlots].reverse().find(event => event.end.dateTime && new Date(event.end.dateTime) <= slotStart);
                    const origin = previousAppointment?.location || startAddress;
                    
                    console.log(`  - Checking slot at ${slotStart.toLocaleTimeString()}. Origin: ${origin}`);
                    
                    const travelTimeSeconds = await getTravelTime(origin, destinationAddress);
                    const travelTimeMinutes = Math.ceil(travelTimeSeconds / 60);

                    const appointmentStartTime = new Date(slotStart.getTime() + travelTimeMinutes * 60000);
                    const appointmentEndTime = new Date(appointmentStartTime.getTime() + appointmentDurationMinutes * 60000);
                    const totalBlockEndTime = new Date(appointmentEndTime.getTime() + bufferMinutes * 60000);
                    ...
                    for (const busy of busySlots) {
                        ...
                        if (appointmentStartTime < busyEnd && totalBlockEndTime > busyStart) {
                            console.log(`  - Slot rejected: conflicts with existing event from ${busyStart.toLocaleTimeString()} to ${busyEnd.toLocaleTimeString()}`);
                            isAvailable = false;
                            break;
                        }
                    }
                    ...
                    if (isAvailable) {
                        console.log(`  - Slot FOUND: ${appointmentStartTime.toISOString()}`);
                        availableSlots.push({ start: appointmentStartTime.toISOString(), end: appointmentEndTime.toISOString() });
                    }
        ...
        console.log(`5. Calculation finished. Found ${availableSlots.length} total available slots.`);
        ...
        res.status(500).send('Er is een kritieke fout opgetreden bij het ophalen van beschikbaarheid.');
    }
});
...
app.post('/book-appointment', async (req, res) => {
    ...
    try {
        ...
        const event = {
            ...
            end: { dateTime: new Date(new Date(startTime).getTime() + duration * 60000).toISOString(), timeZone: 'Europe/Amsterdam' },
...
const startServer = async () => {
    await createTables();
    const requiredVars = ['GOOGLE_CLIENT_ID', 'GOOGLE_CLIENT_SECRET', 'GOOGLE_REDIRECT_URI', 'DATABASE_URL', 'GOOGLE_MAPS_API_KEY'];
